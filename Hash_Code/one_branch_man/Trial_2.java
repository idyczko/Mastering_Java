    ...
    for (int i = 0; i < sortedMoves.size(); i++) {
      for (int j = 0; j < sortedMoves.size(); j++) {
		...
		public MoveCollection nextMove(RoundContext context) {
          if (!firstRound) {
			List<String> attacks = Arrays.stream(context.getLastOpponentMoves().getAttacks()).map(a -> "A"+ myMapping.get(a)).collect(Collectors.toList());
          	Set<String> blocks = Arrays.stream(context.getLastOpponentMoves().getDefences()).map(a ->"D"+ myMapping.get(a)).collect(Collectors.toSet());
