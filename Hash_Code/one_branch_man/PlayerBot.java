...
public class PlayerBot {
  ...
  public static void main(String[] ass) {
    createMatrix();
    List<String> temp = new ArrayList<>(possibleMoves);
    Collections.sort(temp);
    int[][] matrix = new int[temp.size()][temp.size()];
    for (int i = 0; i < temp.size(); i++) {
      for (int j = 0; j < temp.size(); j++) {
        matrix[i][j] = result(temp.get(i), temp.get(j));
      }
    }
    for (int i = 0; i < temp.size(); i++) {
      for (int j = 0; j < temp.size(); j++) {
    ...
    Set<String> nasheq = findNashEquilibrium(temp, matrix);
    System.out.println("Nash:");
    nasheq.forEach(System.out::println);
  }

  private static Set<String> findNashEquilibrium(List<String> moves, int[][] matrix) {
  ...
  private static void createMatrix() {
    fillMatrix(possibleMoves, new ArrayList<>(), new HashSet<>());
  }

  private static void fillMatrix(Set<String> possibleMoves, List<String> attacks, Set<String> blocks) {
    ...
    if (!blocks.contains("DHK")) {
      blocks.add("DHK");
      fillMatrix(possibleMoves, attacks, blocks);
      blocks.remove("DHK");
    }
